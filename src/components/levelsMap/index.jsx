...
export const LevelsMap = ({ currentLevel }) => {
  ...
  const customStyle = useMemo(() => {
    ...
    const mapPadding =
      currentLevel === 1
        ? temp
        : temp - dataCurrentLevel?.width * (currentLevel - 2);
    if (mapPadding) {
  ...
  return (
    <div className="h-[190px] w-full relative ">
      <div
        style={customStyle}
        className={`absolute bg-[#292D38] rounded-[20px] h-full main_block_shadow`}
      >
        <div className="flex items-center justify-start h-full px-6 py-6 space-x-3 ">
          {levelsInfo.map((item, itemIndex) => {
            ...
            return <Level {...item} key={itemIndex} isChooseLevel={isChoose} />;
          })}
        </div>
      </div>
    </div>
